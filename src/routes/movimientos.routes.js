...
const { alertaNuevaSolicitud, alertaNuevaEntrada } = require('../services/telegram.service');
...
router.post('/movimientos', verificarToken, upload.array('fotos', 20), async (req, res) => {
...
if (tipo === 'salida') {
    ...
    const solicitaRes = await client.query('SELECT nombre FROM usuarios WHERE id = $1', [solicitante_id]);
    const ubicacionRes = await client.query('SELECT nombre FROM ubicaciones WHERE id = $1', [ubicacion_destino_id]);
    ...
    await alertaNuevaSolicitud(movimientoConDetalle, { nombre: solicitaRes.rows[0]?.nombre });
